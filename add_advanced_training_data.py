#!/usr/bin/env python3
"""
Add Advanced Training Data for Accuracy Improvement
This script adds high-quality, diverse training data to improve model accuracy
"""
...
def add_advanced_training_data():
    """Add advanced training data to improve model accuracy"""
    try:
        print("Adding advanced training data for accuracy improvement...")
        ...
        if os.path.exists(CSV_FILE):
            ...
            df_existing = pd.DataFrame(columns=['User Input', 'Friend Mode Response', 'Professional Mode Response'])
        
        # Create DataFrame for new advanced data
        df_advanced = pd.DataFrame(advanced_data)
        
        # Combine with existing CSV data, dropping duplicates
        df_combined = pd.concat([df_existing, df_advanced], ignore_index=True)
        
        # Remove duplicates based on user input
        df_combined = df_combined.drop_duplicates(subset=['User Input'], keep='first')
        ...
        print("Advanced training data added successfully!")
        
        return True
        
    except Exception as e:
        print(f"Error adding advanced training data: {e}")
        return False

if __name__ == "__main__":
    success = add_advanced_training_data()
    if success:
        print("\n[SUCCESS] Advanced training data added successfully!")
    else:
        print("\n[ERROR] Failed to add advanced training data!")
